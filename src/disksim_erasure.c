...
static int read_optimal(int w, int k, int p) {
	if (k == p)
		return w == ((1 << p) - 1);
	if (k == p - 1)
		return w == 0;
...
static void erasure_rebuild_init(metadata *meta) {
	...
	int w, mask = (1 << meta->rows) - 1;
	int patt, r, c, p = meta->prime;
...
void erasure_maprequest(metadata *meta, int method, ioreq *req) {
	int numreqs = 0;
