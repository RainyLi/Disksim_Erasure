...
static void logorg_parity_table_dodeps_nosync (logorg *currlogorg, ioreq_event *curr, ioreq_event **redunlist, ioreq_event **reqlist)
{
	...
	//printf("run to here\n");
	//for (i=0; i<currlogorg->actualnumdisks; i++)
	//	for (tmp = *reqlist[i]; tmp != NULL; tmp = tmp->next)
	//		printf("- time=%f devno=%d start=%d length=%d flags=%d\n", tmp->time, tmp->devno, tmp->blkno, tmp->bcount, tmp->flags);

	for (i=0; i<currlogorg->actualnumdisks; i++) {
		temp = redunlist[i];
		while (temp) {
			//printf("? time=%f devno=%d start=%d length=%d flags=%d\n", temp->time, temp->devno, temp->blkno, temp->bcount, temp->flags);
			depend = (depends *) getfromextraq();
			depend->blkno = temp->blkno;
			depend->devno = temp->devno;
			depend->numdeps = 0;
		...
		temp = reqlist[i];
		while (temp) {
			//printf("+ time=%f devno=%d start=%d length=%d opid=%d flags=%d\n", temp->time, temp->devno, temp->blkno, temp->bcount, temp->opid, temp->flags);
			del = temp;
			temp = temp->next;
			//printf("[3] i=%d result=%d,%d\n", i, (redunlist[i] == NULL), (del->opid == 0));
	...
	//printf("print readlist:\n");
	//for (tmp = readlist; tmp != NULL; tmp = tmp->next)
	//	printf("@r time=%f devno=%d start=%d length=%d opid=%d flags=%d\n", tmp->time, tmp->devno, tmp->blkno, tmp->bcount, tmp->opid, tmp->flags);
	//printf("print deplist:\n");
	//for (tmp = deplist; tmp != NULL; tmp = tmp->next)
	//	printf("@d time=%f devno=%d start=%d length=%d opid=%d flags=%d\n", tmp->time, tmp->devno, tmp->blkno, tmp->bcount, tmp->opid, tmp->flags);
	//printf("\n");
	temp = deplist;
	while (temp) {
		holddep = temp->opid;
		...
		while (tmpread) {
			if ((tmpread->devno == temp->devno) | (holddep & tmpread->opid)) {
				//printf("temp.devno=%d blkno=%d ", temp->devno, temp->blkno);
				//printf("tmpread.devno=%d blkno=%d\n", tmpread->devno, tmpread->blkno);
				tmpdep = (depends *) tmpread->prev;
				//printf("  tmpdep.devno=%d blkno=%d numdeps=%d\n", tmpdep->devno, tmpdep->blkno, tmpdep->numdeps);
				ASSERT (tmpdep != NULL);
				// depends list exceeds 10...
				if (tmpdep->numdeps > 0) {
