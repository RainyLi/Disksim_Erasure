...
int logorg_maprequest (logorg **logorgs, int numlogorgs, ioreq_event *curr)
{
	...
	*/
	...
	else if (use_erasure_code(logorgs[logorgno]->reduntype)) {
		numreqs = erasure_maprequest(logorgs[logorgno], curr, numreqs);
	} else {
						...
						int printsizestats)
{
	...
	for (i = 0; i < numlogorgs; i++) {
		...
		// XXX new erasure codes
		else if (use_erasure_code(logorgs[i]->reduntype))
			erasure_initialize(logorgs[i]);
...
int logorg_redun(logorg *result, char *s)
{
	...
	// XXX new erasure codes
	else if(!strcmp(s, "RDP")) {
		result->reduntype = RAID6_RDP;
	} else if(!strcmp(s, "EVENODD")) {
