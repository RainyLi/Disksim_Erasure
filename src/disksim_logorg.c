...
// XXX check prime number when applying code
...
*/
   ...
   else if (reduntype == RAID6_RDP) {
	  numreqs = logorg_raid6_rdp(logorgs[logorgno], curr, numreqs);
   }
   else {
...
*/
   /// Here the link list curr becomes a loop! Make sure curr has the real device number.
   orgdevno = curr->devno;
			...
			int printsizestats)
{
	...
	for (i = 0; i < numlogorgs; i++) {
		...
		if (logorgs[i]->reduntype == SHADOWED) {
		...
		else if ((logorgs[i]->reduntype == PARITY_ROTATED) && (logorg_tabular_rottype(
				logorgs[i]->maptype, 
				logorgs[i]->reduntype, 
				logorgs[i]->rottype, 
			...
			logorgs[i]->maptype = ASIS;
			logorgs[i]->reduntype = PARITY_TABLE;
			logorgs[i]->blksperpart = (int) ((double) 
					logorgs[i]->blksperpart * (double) (logorgs[i]->numdisks - 1) / (double) logorgs[i]->numdisks);
		}
		else if (logorgs[i]->reduntype == PARITY_ROTATED) {
			logorgs[i]->blksperpart = (int) ((double) logorgs[i]->blksperpart
					* (double) (logorgs[i]->numdisks - 1) 
					/ (double) logorgs[i]->numdisks) - logorgs[i]->parityunit;
		}
		// XXX new erasure codes
		else if (logorgs[i]->reduntype == RAID7) {
			logorgs[i]->maptype = ASIS;
			logorgs[i]->addrbyparts = FALSE;
			logorgs[i]->numdisks -= 3;
		}
		else if (logorgs[i]->reduntype == RAID6_RDP) {
			if (!isprime(logorgs[i]->actualnumdisks - 1)) {
				fprintf(stderr, "Number of disks in RDP should be p+1 where p is a prime.\n");
				exit(1);
			}
			logorgs[i]->maptype = ASIS;
			logorgs[i]->addrbyparts = FALSE;
			logorgs[i]->numdisks -= 2;
		}
		...
		logorgs[i]->sizes = DISKSIM_malloc(logorgs[i]->actualnumdisks * sizeof(int));

		logorgs[i]->redunsizes = DISKSIM_malloc(logorgs[i]->actualnumdisks * sizeof(int));

		if (logorgs[i]->printinterferestats) {
			logorgs[i]->stat.lastreq = DISKSIM_malloc(2 * INTERFEREMAX*sizeof(int));

			for (j=0; j<(2*INTERFEREMAX); j++) {
				logorgs[i]->stat.lastreq[j] = 0;
			}

			logorgs[i]->stat.intdist = DISKSIM_malloc(INTERFEREMAX*INTDISTMAX*sizeof(int));
		}
...
int logorg_redun(logorg *result, char *s) {
  ...
  // XXX new erasure codes
  else if(!strcmp(s, "Raid6_rdp")) {
  	result->reduntype = RAID6_RDP;
  }
  else if(!strcmp(s, "Raid6_evenodd")) {
  	result->reduntype = RAID6_EVENODD;
  }
  else if(!strcmp(s, "Raid6_hcode")) {
  	result->reduntype = RAID6_HCODE;
  }
  else if(!strcmp(s, "Raid6_xcode")) {
  	result->reduntype = RAID6_XCODE;
  }
  else if(!strcmp(s, "Raid7")) {
  	result->reduntype = RAID7;
  } else {
