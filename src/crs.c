...
int cauchy_setup_tables(int w)
{
	...
	switch (w) {
	case 3: prim_poly = 013; cbest = cbest_3; break;
	case 4: prim_poly = 023; cbest = cbest_4; break;
	case 5: prim_poly = 045; cbest = cbest_5; break;
	case 6: prim_poly = 0103; cbest = cbest_6; break;
	case 7: prim_poly = 0211; cbest = cbest_7; break;
	case 8: prim_poly = 0435; cbest = cbest_8; break;
	case 9: prim_poly = 01021; cbest = cbest_9; break;
	case 10: prim_poly = 02011; cbest = cbest_10; break;
	default: return -1;
	}
	glb_w = w;
	x_to_w = 1 << w;
...
#define NW (1 << glb_w) /* In other words, NW equals 2 to the w-th power */
...
int powo(int a, int b)
{
	int ret = 1;
	while (b--)
		ret = cauchy_mult(ret, a);
	return ret;
}

int print_word(int a)
{
	int i;
	for (i = 0; i < glb_w; i++)
		printf("%d", 1 & (a >> i));
...
void cauchy_improve_coding_matrix(int k, int m, int w, int *matrix)
{
  ...
  for (j = 0; j < k; j++) {
    if (matrix[j] != 1) {
      ...
      index = j;
      for (i = 0; i < m; i++) {
        matrix[index] = cauchy_mult(matrix[index], tmp);
        index += k;
      }
    }
  }
  for (i = 1; i < m; i++) {
    ...
    for (j = 0; j < k; j++) {
      if (matrix[index+j] != 1) {
        tmp = divi(1, matrix[index+j]);
        tno = 0;
        for (x = 0; x < k; x++) {
          tno += cauchy_n_ones(cauchy_mult(matrix[index+x], tmp), w);
        }
        if (tno < bno) {
          bno = tno;
          bno_index = j;
    ...
    if (bno_index != -1) {
      tmp = divi(1, matrix[index+bno_index]);
      for (j = 0; j < k; j++) {
        matrix[index+j] = cauchy_mult(matrix[index+j], tmp);
      }
    }
  }
}
